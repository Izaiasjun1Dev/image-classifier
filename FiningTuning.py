...
# Pandas para tratar data frames de dados gerados em algumas parte do treinamento
import pandas as pd
...
if __name__ == '__main__':
    ...
    models_path = "TuningModel"  # Diretorio para os modelos treinados
    ...
    # Intanciando arquitetura pré-executada
    model = create_model_arquitecture(input_shape)

    # Definindo otimizador do modelo
    opt = Adam(lr=init_lr, decay=init_lr / epochs)


    """Congelamento de algumas camadas de treinamento do modelo"""
    ...
    predict_layers = tf.keras.layers.Dense(
        2, activation='softmax')(average_layer)
    ...
    checkpoint1 = ModelCheckpoint(
        models_path, monitor='val_acc', verbose=1, save_best_only=True, mode='max')
    checkpoint2 = ModelCheckpoint(
        models_path, monitor='val_loss', verbose=1, save_best_only=True, mode='min')
    ...
    """Após o primeiro treinamento ultilizando arquitetura imagenet transfer learning 
    descongelei algumas camadas do modelo que servirão para o ajuste um fino"""

    model.trainable = True
    fine_tuning_at = 100

    for layers in model.layers[:fine_tuning_at]:
        layers.trainable = False

    # Recompila o modelo com base nos novos ajustes
    model.compile(
    ...
    # Efetua-se um novo treinamento com base no ajuste fino feito
    history = model.fit_generator(
        aug.flow(
                trainX, 
                trainY, 
    ...
    print(hist_df)
    hist_csv_file = "./Csvs/Cvs_epochs_10/fine_tuning.csv"
